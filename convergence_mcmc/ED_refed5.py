...
import os
my_path = '/fslhome/fslcollab151/convergence_mcmc/Results/refed5'
print(my_path)
class diagnostics:
...
def li(x):
    ...
    states = odeint(f, state0, t, args = (para,))
    u0 = np.mean(states[2])
    u1 = np.mean(states[0]*states[0])
    u2 = np.mean(states[0]*states[1])
    u3 = np.mean(states[2]*states[2])
    u4 = np.mean(states[0]*states[1]*states[2])
    u5 = np.mean(states[2]*states[2]*states[2])
    u6 = np.mean(states[0]*states[1]*states[2]*states[2])
    return norm.pdf(u0, z_true, np.absolute(z_true)/10.0) * norm.pdf(u1, x2_true, np.absolute(x2_true)/10.0) * norm.pdf(u2, xy_true, np.absolute(xy_true)/10.0) * norm.pdf(u3, z2_true, np.absolute(z2_true)/10.0) * norm.pdf(u4, xyz_true, np.absolute(xyz_true)/10.0) * norm.pdf(u5, z3_true, np.absolute(z3_true)/10.0) * norm.pdf(u6, xyz2_true, np.absolute(xyz2_true)/10.0)
