...
import os
my_path = '/fslhome/fslcollab151/convergence_mcmc/Results/10000z2xz'
print(my_path)
class diagnostics:
...
def li(x):
    para = x
    states = odeint(f, state0, t, args = (para,))
    u0=np.mean(states[2]*states[2])
    u1=np.mean(states[0]*states[2])    
    return norm.pdf(u0, z2_true, 1.0) * norm.pdf(u1, xz_true, 1.0) 
...
np.savetxt('z2xz_x.out',all_samples_x, delimiter=',')
np.savetxt('z2xz_y.out',all_samples_y, delimiter=',')
np.savetxt('z2xz_z.out',all_samples_y, delimiter=',')
