...
import os
my_path = '/fslhome/fslcollab151/convergence_mcmc/Results/hmc'
class diagnostics:
...
x_mean, y_mean, z_mean = np.mean(states, axis = 0)
...
def li(x):
    ...
    u1=np.mean(states[1])
    u2=np.mean(states[2])
    return norm.pdf(u0, x_mean, x_var) * norm.pdf(u1, y_mean, y_var) * norm.pdf(u2, z_mean, z_var) 

def U(x):
    return -np.log(pr(x) * li(x))
def dU(x):
    return np.array([(x[0] - x_mean)/x_var, (x[1] - y_mean)/y_var, (x[2] - z_mean)/z_var])
...
for j in range(10):
    ...
    for i in range(n):
        ...
        current_K = np.matmul(np.transpose(current_p),current_p)/2
        proposed_U = U(q)
        proposed_K = np.matmul(np.transpose(p),p)/2
        
        alpha = min(1, np.exp(current_U+current_K-proposed_U-proposed_K))
        u = np.random.uniform(0, 1)
        q_next = q if u < alpha else current_q
