...
import os
"""
The difference between ed2 and ed1 is in the li() the variance is true value/4
"""
my_path = '/fslhome/fslcollab151/convergence_mcmc/Results/12dnged2'
print(my_path)
class diagnostics:
...
# solve ODE to get the true values of x,y,z
def f(state, t, para):
...
#likelyhood of parameters given 1&2d obeserbations
def li(x):
    para = x
    ...
    return norm.pdf(u0, x_true, np.absolute(x_true)/4.0) * norm.pdf(u1, y_true, np.absolute(y_true)/4.0) * norm.pdf(u2, z_true, np.absolute(z_true)/4.0) * norm.pdf(u3, x2_true, np.absolute(x2_true)/4.0) * norm.pdf(u4, y2_true, np.absolute(y2_true)/4.0) * norm.pdf(u5, z2_true, np.absolute(z2_true)/4.0) * norm.pdf(u6, xy_true, np.absolute(xy_true)/4.0) * norm.pdf(u7, xz_true, np.absolute(xz_true)/4.0) * norm.pdf(u8, yz_true, np.absolute(yz_true)/4.0)
...
for j in range(10):
    ...
    print("Acceptance rate:", len(set(x))/len(x))
# Plot the samples fo sigma, rho and beta
    fig1 = plt.figure()
    plt.hist(x, bins=100, density=True)
    myfile_x = 'sigma'+str(j) 
...
"""
diagno4 = diagnostics(all_samples_xb)
diagno4.Gelman_Rubin()    
diagno4.geweke()
diagno5 = diagnostics(all_samples_yb)
diagno5.Gelman_Rubin()    
diagno5.geweke()
diagno6 = diagnostics(all_samples_zb)
diagno6.Gelman_Rubin()    
diagno6.geweke()
