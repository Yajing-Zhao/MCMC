...
import os
my_path = '/fslhome/fslcollab151/convergence_mcmc/Results/10000y2xy'
print(my_path)
class diagnostics:
...
def li(x):
    para = x
    states = odeint(f, state0, t, args = (para,))
    u0=np.mean(states[1]*states[1])
    u1=np.mean(states[0]*states[1])    
    return norm.pdf(u0, y2_true, 1.0) * norm.pdf(u1, xy_true, 1.0) 
...
np.savetxt('y2xy_x.out',all_samples_x, delimiter=',')
np.savetxt('y2xy_y.out',all_samples_y, delimiter=',')
np.savetxt('y2xy_z.out',all_samples_y, delimiter=',')
