...
import os
my_path = '/fslhome/fslcollab151/convergence_mcmc/Results/12ded5'
print(my_path)
class diagnostics:
...
def li(x):
    ...
    return norm.pdf(u0, x_true, np.absolute(x_true)/10.0) * norm.pdf(u1, y_true, np.absolute(y_true)/10.0) * norm.pdf(u2, z_true, np.absolute(z_true)/10.0) * norm.pdf(u3, x2_true, np.absolute(x2_true)/10.0) * norm.pdf(u4, y2_true, np.absolute(y2_true)/10.0) * norm.pdf(u5, z2_true, np.absolute(z2_true)/10.0) * norm.pdf(u6, xy_true, np.absolute(xy_true)/10.0) * norm.pdf(u7, xz_true, np.absolute(xz_true)/10.0) * norm.pdf(u8, yz_true, np.absolute(yz_true)/10.0)
