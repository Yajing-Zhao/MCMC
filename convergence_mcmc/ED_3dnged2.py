...
import os
my_path = '/fslhome/fslcollab151/convergence_mcmc/Results/3dnged2'
print(my_path)
class diagnostics:
...
def li(x):
    ...
    u0 = np.mean(states[3]*states[3]*states[3])
    u1 = np.mean(states[4]*states[4]*states[4])
    u2 = np.mean(states[5]*states[5]*states[5])
    u3 = np.mean(states[3]*states[3]*states[4])
    u4 = np.mean(states[3]*states[3]*states[5])
    u5 = np.mean(states[3]*states[4]*states[4])
    u6 = np.mean(states[4]*states[4]*states[5])
    u7 = np.mean(states[3]*states[5]*states[5])
    u8 = np.mean(states[4]*states[5]*states[5])
    return norm.pdf(u0, x3_true, np.absolute(x3_true)/4.0) * norm.pdf(u1, y3_true, np.absolute(y3_true)/4.0) * norm.pdf(u2, z3_true, np.absolute(z3_true)/4.0) * norm.pdf(u3, x2y_true, np.absolute(x2y_true)/4.0) * norm.pdf(u4, x2z_true, np.absolute(x2z_true)/4.0) * norm.pdf(u5, y2x_true, np.absolute(y2x_true)/4.0) * norm.pdf(u6, y2z_true, np.absolute(y2z_true)/4.0) * norm.pdf(u7, z2x_true, np.absolute(z2x_true)/4.0) * norm.pdf(u8, z2y_true, np.absolute(z2y_true)/4.0)
