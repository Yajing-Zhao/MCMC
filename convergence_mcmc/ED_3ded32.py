...
import os
my_path = '/fslhome/fslcollab151/convergence_mcmc/Results/3ded32'
print(my_path)
class diagnostics:
...
def li(x):
    para = x
    states = odeint(f, state0, t, args = (para,))
    u0 = np.mean(states[0]*states[0]*states[0])
    u1= np.mean(states[1]*states[1]*states[1])
    u2 = np.mean(states[2]*states[2]*states[2])
    u3 = np.mean(states[0]*states[0]*states[1])
    u4 = np.mean(states[0]*states[0]*states[2])
    u5 = np.mean(states[0]*states[1]*states[1])
    u6 = np.mean(states[1]*states[1]*states[2])
    u7 = np.mean(states[0]*states[2]*states[2])
    u8 = np.mean(states[1]*states[2]*states[2])
    return norm.pdf(u0, x3_true, x3_var) * norm.pdf(u1, y3_true, y3_var) * norm.pdf(u2, z3_true, z3_var) * norm.pdf(u3, x2y_true, x2y_var) * norm.pdf(u4, x2z_true, x2z_var) * norm.pdf(u5, y2x_true, y2x_var) * norm.pdf(u6, y2z_true, y2z_var) * norm.pdf(u7, z2x_true, z2x_var) * norm.pdf(u8, z2y_true, z2y_var)
